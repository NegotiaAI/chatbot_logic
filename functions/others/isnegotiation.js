function isNegotiation(userintent,generalintent) {
    
  const negotiationIntents= ["insist","vague-price","counter-price", "offer","intro","unknown", "init-price","inquiry","inform", "disagree","agree"]
  ...
  const isUserIntent=negotiationIntents.includes(userIntent)



const excludedIntents = ["intro", "unknown", "inquiry"];

const filteredIntents = negotiationIntents.filter(intent => !excludedIntents.includes(intent));

//   console.log(userintent,generalintent)
//   console.log(userIntent,userIntentConfidence,generalIntent,generalIntentConfidence,isGeneralIntent)

  if(userIntent=="intro" && userIntentConfidence>0.5)
    {
         return false
    }
    else if(userIntent=="inquiry" && userIntentConfidence>0.5 && generalIntentConfidence>0.5)
    {
        return false
    }
    else if(isGeneralIntent && generalIntentConfidence>0.5 && !filteredIntents.includes(userIntent))
    {
      return false
    }
    else if(userIntent=="inquiry" && generalIntentConfidence>0.4 && isGeneralIntent)
    {
        return false
    }
    else if(userIntent=="inquiry" && userIntentConfidence>0.5 && generalIntentConfidence<0.5)
